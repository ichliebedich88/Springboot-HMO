...
//                    System.out.println("output:" + object);
 ...
 //           return Result.success().add("data",returnJson);
    ...
    @RequestMapping(value = "/dag/queryUnboundDag")
    public Result ggetAllDagsNotBinding(){
        if (dagService.getAllDagsNotBinding()!=null) {
            List<Dag> allDags = dagService.getAllDagsNotBinding();
            List<Object> data = new ArrayList();
            for(Dag dag:allDags)
            {
                DagNotBind dagNB = new DagNotBind();
                Integer id = dag.getId();
                dagNB.setId(id);
                dagNB.setName(dag.getDescrib());
                //查tasks
                if (taskService.getTasksByDagId(id)!=null){
                    List<Object> arrayList = new ArrayList();
                    List<Task> tasks  = taskService.getTasksByDagId(id);
                    for(int i=0;i<tasks.size();i++){
                        arrayList.add(tasks.get(i).getName());
                    }
                    dagNB.setTasks(arrayList.toString());
                }
                else
                    dagNB.setTasks(null);
                data.add(dagNB);
    ...
    @RequestMapping(value = "/dag/ajax_binding")
    public Result dag_binding(@RequestParam("id") Integer id){
        //dag所需资源类型
        List<Task> task_list = taskService.getTasksByDagId(id);
        List<String> require_list = new ArrayList<String>();
        for(Task task:task_list)
            require_list.add(task.getType());

        int binding_room_id = 0,satisfy = 0;
        //获取未绑定的房间
        List<Room> room_list = roomService.getRoomsFree();
        for(Room room:room_list)
        {
            List<Resource> res_list = resourceService.getResourcesInRoom(room.getId());
            //获取房间里的资源类型
            List<String> types = new ArrayList<String>();
            for (Resource res:res_list)
                types.add(res.getType());
            int cur = 0;
            for (String require:require_list)
            {
            ...
            if (cur>satisfy)
            {
                binding_room_id = room.getId();
                satisfy = cur;
            }
        }
        if(binding_room_id == 0)
            binding_room_id = room_list.get(0).getId();

        if(dagService.giveRoomForDag(id,binding_room_id)<=0)
            return Result.error();

        if(roomService.bindRoom(id)<=0)
        {
            dagService.giveRoomForDag(id, null);
            return Result.error();
        }
        return Result.success().add("room_id",binding_room_id);
    }
}
